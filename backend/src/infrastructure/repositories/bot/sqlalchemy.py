...
class SQLAlchemyBotRepository(BaseBotRepository):
    ...
    async def get_all_bots_with_rentals(self) -> list[BotEntity] | None:
        try:
            ...
            logger.error(f'Ошибка при получении ботов с арендаторами: {e}', exc_info=True)
            raise
    ...
    async def get_bot_with_rentals(self, bot_id: int) -> BotEntity | None:
        try:
            ...
            logger.error(f'Ошибка при получении бота с арендаторами id={bot_id}: {e}', exc_info=True)
            raise
