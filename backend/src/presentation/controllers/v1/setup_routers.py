...
def setup_controllers(app: FastAPI):
    ...
    app.include_router(admins_monitoring_router, prefix='/api/v1', tags=['Мониторинг Админ'])
    app.include_router(user_router, prefix='/api/v1/users', tags=['Пользователи'])
    app.include_router(admin_users_router, prefix='/api/v1/users', tags=['Пользователи Админ'])
