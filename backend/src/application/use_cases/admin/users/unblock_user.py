...
class UnblockUserUseCase:
    ...
    async def execute(self, telegram_id: int, admin: UserEntity) -> bool:
        ...
        logger.info(f'Администратор: {admin.telegram_id.to_raw()} разблокировал пользователя: {user.telegram_id.to_raw()}')
        await send_notification_for_admin.kiq(text=f'Администратор: {admin.telegram_id.to_raw()} разблокировал пользователя: {user.telegram_id.to_raw()}')
        return True
