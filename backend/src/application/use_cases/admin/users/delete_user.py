...
class DeleteUserUseCase:
    ...
    async def execute(self, telegram_id: int, admin: UserEntity) -> UserEntity | None:
        ...
        if user.telegram_id.to_raw() == admin.telegram_id.to_raw():
            logger.warning(f'Администратор: {admin.telegram_id.to_raw()} попытался удалить сам себя')
            await send_notification_for_admin.kiq(text=f'Администратор: {admin.telegram_id.to_raw()} попытался удалить сам себя')
        ...
        if user.role in ['dev', 'admin']:
            ...
            await send_notification_for_admin.kiq(text=f'Администратор: {admin.telegram_id.to_raw()} попытался удалить администратора: {user.telegram_id.to_raw()}')
            raise PermissionDeniedException()
        ...
        if not user.is_deleted:
            ...
            await send_notification_for_admin.kiq(text=f'Администратор: {admin.telegram_id.to_raw()} удалил пользователя: {user.telegram_id.to_raw()}')
            return deleted_user
